...
define(["exports", "dojo/dom-construct"], function (exports, domConstruct) {
  ...
  function sampleProceduralRGB(uvVec, color) {
    ...
    rgba[0] = (Math.random() * 255) >> 0;
    rgba[1] = (Math.random() * 255) >> 0;
    rgba[2] = ((1 - Math.random() / 2 + Math.random() / 2) * 0.5 * 255) >> 0;
    rgba[3] = 255;

    color.rgbaShaded = rgba;
  ...
  function create(textureURL, callback) {
    ...
    texture.image.onload = function () {
      ...
      texture.canvas = domConstruct.create(
        "canvas",
      ...
      try {
        texture.imageData = ctx.getImageData(
          0,
          0,
          texture.width,
          texture.height
